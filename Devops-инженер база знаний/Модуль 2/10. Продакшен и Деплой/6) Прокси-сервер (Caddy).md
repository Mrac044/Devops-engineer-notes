...
![[Pasted image 20260512164653.png]]

Часто такой веб-сервер либо встроен в сам язык как модуль, либо написан на этом же языке, для запуска кода внутри себя. Обычно, задачу интеграции берут на себя фреймворки. В нашем приложении [devops-example-app](https://github.com/hexlet-components/devops-example-app) за это отвечает фреймворк [fastify](https://www.fastify.io/). Внутри себя он [использует встроенный](https://github.com/fastify/fastify/blob/main/lib/server.js#L210) в Node.js веб-сервер:
...
Несмотря на простоту такой схемы, ее в большинстве проектов недостаточно. Помимо встроенного веб-сервера, нужен внешний, выполняющий роль реверс-прокси (reverse-proxy), например [Nginx](https://www.nginx.com/) или [Caddy](https://caddyserver.com/).

![[Pasted image 20260512164811.png]]
...
Только какой выбрать? Nginx самый популярный сервер в мире, но Caddy, гораздо проще в настройке и отладке.

Изучив его, вы без труда сможете перейти на Nginx, так как концептуально все подобные веб-сервера работают одинаково.
...
Мы придерживаемся структуры, в которой основное приложение располагается в корне проекта, а дополнительные сервисы лежат в директории _services_. У каждого сервиса своя директория с Dockerfile внутри.
...
После этого запрос к _localhost_ пойдет в Caddy, который выполнит автоматическое перенаправление на _https_.
...
Каждый контейнер в Docker запускается в изолированной среде и не знает про другие контейнеры. Для их связи используется **network**.
...
Наш проект выложен на сервер и работает. Технически мы можем обратиться к нему по ip-адресу, но так делать не стоит. ip-адрес всегда меняется при пересоздании сервера, из-за чего постоянно придется запоминать новый.

==Лучше иметь доменное имя, которое никогда не меняется.==
...
- [Как работает DNS](https://ru.hexlet.io/blog/posts/chto-takoe-dns-server-prostymi-slovami)
    
- [Что такое обратный прокси-сервер](https://ru.hexlet.io/blog/posts/obratnyy-proksi)