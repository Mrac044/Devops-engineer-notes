В таком случае код вставки пользователя выглядит так:
...
==Запросы составленные таким образом несут в себе серьезную опасность называемую SQL-инъекцией.==

Представьте что внутри переменной бы хранилась строка `') INSERT INTO users (username, phone) VALUES ('i am hacker', '777777')`. В таком случае, после подстановки мы бы получили следующий запрос:
...
#Реальная_работа В разработке существует правило - никогда не доверяй пользовательским данным. В случае работы с базой данных, это особенно важно. Любые данные перед вставкой должны быть экранированы. Делается это с помощью специальных плейсхолдеров:
...
- [Передача параметров](https://www.psycopg.org/docs/usage.html#passing-parameters-to-sql-queries)